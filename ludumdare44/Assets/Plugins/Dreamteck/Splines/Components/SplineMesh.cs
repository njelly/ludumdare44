...
namespace Dreamteck.Splines
{
    ...
    public class SplineMesh : MeshGenerator
    {
        ...
        private void Place(Channel channel, TS_Mesh target, double percent)
        {
            ...
            Vector2 channeloffset = channel.NextOffset();
            ...
            vertexMatrix.SetTRS(result.position + originalRight * (offset.x + channeloffset.x) * result.size + originalNormal * (offset.y + channeloffset.y) * result.size + originalDirection * offset.z, result.rotation * channel.NextPlaceRotation() * Quaternion.AngleAxis(rotation, Vector3.forward), channel.NextPlaceScale() * result.size);
        ...
        private void Stretch(Channel channel, TS_Mesh target, double from, double to)
        {
            ...
            definition.Write(target, channel.overrideMaterialID ? channel.targetMaterialID : -1);
            Vector2 channelOffset = channel.NextOffset();
            Vector3 channelScale = channel.NextExtrudeScale();
            float channelRotation = channel.NextExtrudeRotation();
